...
*/
  ...
  public class PageFault {
    ...
    public static void replacePage ( Vector mem , int virtPageNum , int replacePageNum, String output, HashMap physicalMem, String replacementAlgo ) {
      switch (replacementAlgo) {
        ...
        case "Clock_policy":
          ...
          Clock_policy(mem, virtPageNum, replacePageNum, output);
          break;
        default:
    ...
    public static void Clock_policy(Vector mem, int virtPageNum, int replacePageNum, String output) {
      int hand = 0;
      ...
      try (PrintStream log = new PrintStream(new FileOutputStream("log.txt", true))) {
        log.println("Before replacement:");
        for (int i = 0; i < pages.size(); i++) {
          Page page = (Page) pages.elementAt(i);
          log.println("Page: " + page.id + " in memory: " + page.physical + " last touch time: " + page.lastTouchTime + " in memory time: " + page.inMemTime + " reference bit: " + page.reference_bit);
        }
        log.println();
